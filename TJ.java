...
public class TJ {
	   ...
	   public static class Map extends MapReduceBase implements Mapper <LongWritable, Text, Text,Text> {
		     public void map(LongWritable key, Text value, OutputCollector<Text,Text> output, Reporter reporter) throws IOException {//map函数两个输入两个输出
		    	 
		       String line = value.toString();//得到行数据并赋值给line
		       
		       String ii=((FileSplit)reporter.getInputSplit()).getPath().getName().toString();//读取文件名
		       ...
		       Text word = new Text();
		       StringTokenizer tokenizer = new StringTokenizer(line);//字符串放入tokenizer分子串
		       ...
		       if(j!=0){
			       kkb=Math.abs(vother) %5; //hash function
...
//		    	 }
		    	 ...
		    	 ArrayList<String> al=new ArrayList();//数组
		    	 
		    	 while (values.hasNext()) {
		    		 String record = values.next().toString();
		    		 String[] rrecord=new String[3];//取value
